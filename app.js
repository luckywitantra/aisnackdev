...
refreshData: function() {
   ...
   filterProducts: function(key) {
    ...
    createProductCard: function(p) {
        ...
        // Menghapus h-[200px] dan menggantinya dengan h-full agar tidak menindih baris bawahnya
        return `<div onclick="${p.maxStok > 0 ? `superApp.addToCart('${p.sku}', '${p.nama}', ${p.harga}, ${p.maxStok}, '${p.sku_bahan || ''}', event)` : ''}" 
            class="bg-white border-2 border-transparent rounded-2xl md:rounded-[1.5rem] cursor-pointer shadow-sm md:shadow-[0_4px_15px_rgba(0,0,0,0.04)] transition-all duration-300 flex flex-col relative group ${isHabis} overflow-hidden h-full"> 
            ...
            <div class="aspect-[4/3] w-full overflow-hidden bg-slate-100 relative shrink-0 border-b border-slate-50">
                ${img}
                ${overlayQty}
            </div>
            
            <div class="flex flex-col flex-1 justify-between p-2 md:p-3 bg-white w-full">
                <h3 class="font-bold text-[11px] md:text-sm text-slate-800 leading-snug break-words mb-2" style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
                    ${namaProduk}
                </h3>
